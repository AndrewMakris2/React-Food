...
const MACRO_STYLES = {
  protein:  { border:'border-violet-200 dark:border-violet-700/40', bg:'bg-violet-50 dark:bg-violet-900/10', label:'text-violet-700 dark:text-violet-400', toggle:'bg-violet-500', preset:'bg-violet-100 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300', presetActive:'bg-violet-500 text-white' },
  calories: { border:'border-orange-200 dark:border-orange-700/40', bg:'bg-orange-50 dark:bg-orange-900/10', label:'text-orange-700 dark:text-orange-400', toggle:'bg-orange-500', preset:'bg-orange-100 dark:bg-orange-900/30 text-orange-700 dark:text-orange-300', presetActive:'bg-orange-500 text-white' },
  carbs:    { border:'border-amber-200 dark:border-amber-700/40',   bg:'bg-amber-50 dark:bg-amber-900/10',   label:'text-amber-700 dark:text-amber-400',   toggle:'bg-amber-500',  preset:'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300',   presetActive:'bg-amber-500 text-white'  },
  fat:      { border:'border-rose-200 dark:border-rose-700/40',     bg:'bg-rose-50 dark:bg-rose-900/10',     label:'text-rose-700 dark:text-rose-400',     toggle:'bg-rose-500',   preset:'bg-rose-100 dark:bg-rose-900/30 text-rose-700 dark:text-rose-300',     presetActive:'bg-rose-500 text-white'   },
}
...
function SectionDivider({ label }) {
  return (
    <div className="flex items-center gap-2 mb-3 sm:mb-4">
      <span className="text-[10px] sm:text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest whitespace-nowrap">{label}</span>
  ...
  return (
    <div className={`rounded-xl sm:rounded-2xl border p-3 sm:p-4 transition-all duration-200 ${
      ...
      <div className="flex items-center justify-between mb-2 sm:mb-3">
        <div className={`flex items-center gap-1 sm:gap-1.5 font-semibold text-[10px] sm:text-xs uppercase tracking-widest ${active ? s.label : 'text-slate-400 dark:text-slate-500'}`}>
          <Icon className="w-3 h-3 sm:w-3.5 sm:h-3.5 flex-shrink-0" />
          <span>{label}</span>
        </div>
        {!required && (
          <button onClick={onToggle} className={`relative w-8 h-4 sm:w-9 sm:h-5 rounded-full transition-colors duration-200 flex-shrink-0 ${enabled ? s.toggle : 'bg-slate-300 dark:bg-slate-600'}`} aria-label={`Toggle ${label}`}>
            <span className={`absolute top-0.5 w-3 h-3 sm:w-4 sm:h-4 rounded-full bg-white shadow transition-transform duration-200 ${enabled ? 'translate-x-4' : 'translate-x-0.5'}`} />
          </button>
        )}
      </div>
      {active ? (
        <>
          <div className="flex items-baseline gap-1">
            <input type="number" inputMode="numeric" value={value} onChange={e => onChange(e.target.value)} min="0"
              className="w-full bg-transparent text-xl sm:text-2xl font-bold text-slate-900 dark:text-white focus:outline-none placeholder-slate-300 dark:placeholder-slate-600" placeholder="—" />
          ...
          {presets && (
            <div className="flex gap-1 mt-2 sm:mt-3">
              {presets.map(p => (
                <button key={p} onClick={() => onChange(String(p))}
                  className={`flex-1 py-0.5 sm:py-1 rounded-md text-[10px] sm:text-[11px] font-semibold transition-colors min-h-[24px] ${value === String(p) ? s.presetActive : s.preset}`}>
                  {p}
                </button>
              ))}
            </div>
...
export default function MealGenerator({ generatedMeal, setGeneratedMeal, onSaved }) {
  ...
  const [regenLoading,  setRegenLoading]  = useState(new Set()) // Set of meal indices being regenerated
  ...
  // ── TDEE apply ────────────────────────────────────────────────
  function applyTDEE({ protein, calories, carbs, fat }) {
  ...
  function resetFilters() {
    setProteinGoal(DEFAULTS.proteinGoal); setMaxCalories(DEFAULTS.maxCalories)
    setMaxCarbs(DEFAULTS.maxCarbs);       setMaxFat(DEFAULTS.maxFat)
    setCaloriesOn(false); setCarbsOn(false); setFatOn(false)
    setMealType(DEFAULTS.mealType); setCuisine(DEFAULTS.cuisine)
    setProteinSource(DEFAULTS.proteinSource); setMealTime(DEFAULTS.mealTime)
    setCookTime(DEFAULTS.cookTime); setSpiceLevel(DEFAULTS.spiceLevel)
    setServings(DEFAULTS.servings); setResultCount(DEFAULTS.resultCount)
    setRestrictions(DEFAULTS.restrictions); setError(null)
  }
  ...
  async function generate() {
    const protein = parseInt(proteinGoal)
    if (!protein || protein < 5) { setError('Please enter a protein goal of at least 5g.'); return }

    abortRef.current?.abort()
    abortRef.current = new AbortController()

    setLoading(true); setError(null); setSaved(false)
    setSavedSet(new Set()); setGeneratedMeal(null); setTipIndex(0)
  ...
  async function regenerateOne(index) {
    if (!lastParamsRef.current || !generatedMeal?.meals) return
    setRegenLoading(prev => new Set([...prev, index]))

    try {
      const res  = await fetch('/api/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...lastParamsRef.current, resultCount: 1 }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || 'Failed')
      if (data.type === 'home_cooked' && data.meals?.[0]) {
        setGeneratedMeal(prev => ({
          ...prev,
          meals: prev.meals.map((m, i) => i === index ? data.meals[0] : m),
        }))
        // Clear saved state for that index
        setSavedSet(prev => { const next = new Set(prev); next.delete(index); return next })
      }
    } catch { /* silently fail — user can try again */ }
    finally {
      setRegenLoading(prev => { const next = new Set(prev); next.delete(index); return next })
    }
  }
  ...
  function saveRecipe() {
    ...
    addRecipe(generatedMeal); setSaved(true); onSaved?.()
  }
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      {/* TDEE Calculator */}
      <TDEECalculator onApply={applyTDEE} />
        ...
        <div className="mt-4 pt-4 border-t border-white/20 flex items-center gap-3 flex-wrap">
          ...
          {location ? (
            <span className="flex items-center gap-1.5 text-xs text-indigo-100">
              ...
              {location}
              <button onClick={() => setLocation(null)} className="text-indigo-300 hover:text-white ml-1 text-[11px] underline">clear</button>
            </span>
          ) : (
            <span className="text-indigo-200/70 text-[11px]">Optional — helps tailor fast food chain recommendations</span>
          )}
        </div>
      </div>
      ...
      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-sm rounded-xl sm:rounded-2xl border border-slate-200/80 dark:border-slate-700/50 p-4 sm:p-6 shadow-sm">
        ...
        <div className="mb-5 sm:mb-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
            <MacroInput id="protein"  icon={Dumbbell} label="Protein"  unit="g"    hint="Min target" value={proteinGoal}  onChange={setProteinGoal}  required enabled presets={[30,50,75,100]} />
            <MacroInput id="calories" icon={Flame}    label="Calories" unit="kcal" hint="Max limit"  value={maxCalories}  onChange={setMaxCalories}  required={false} enabled={caloriesOn} onToggle={() => setCaloriesOn(v=>!v)} presets={[400,600,800]} />
            <MacroInput id="carbs"    icon={Wheat}    label="Carbs"    unit="g"    hint="Max limit"  value={maxCarbs}     onChange={setMaxCarbs}     required={false} enabled={carbsOn}    onToggle={() => setCarbsOn(v=>!v)}    presets={[30,50,80]} />
            <MacroInput id="fat"      icon={Droplets} label="Fat"      unit="g"    hint="Max limit"  value={maxFat}       onChange={setMaxFat}       required={false} enabled={fatOn}      onToggle={() => setFatOn(v=>!v)}      presets={[20,35,50]} />
          </div>
        </div>
                ...
                {SERVINGS.map(s => (
                  <button key={s} onClick={() => setServings(s)}
                    className={`w-10 h-8 rounded-lg text-xs font-semibold border transition-all min-h-[32px] ${
                      servings === s
                        ? 'bg-indigo-500 border-indigo-500 text-white shadow-sm shadow-indigo-500/25'
                        : 'bg-white dark:bg-slate-800/60 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-indigo-300 dark:hover:border-indigo-600'
                    }`}>
                    {s}
                  </button>
                ))}
            ...
            <div className="space-y-3 sm:space-y-4">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-base sm:text-lg font-extrabold text-indigo-500 tabular-nums w-6 text-right">{resultCount}</span>
                </div>
                <input type="range" min="1" max="10" value={resultCount} onChange={e => setResultCount(Number(e.target.value))}
        ...
        <div className="mt-5 sm:mt-6 flex gap-2.5">
          <button onClick={generate} disabled={loading}
            ...
            {loading ? (
              <><Loader2 className="w-5 h-5 animate-spin flex-shrink-0" /><span>{LOADING_TIPS[tipIndex]}</span></>
            ) : (
              <><Zap className="w-5 h-5 flex-shrink-0" />Generate Meal</>
            )}
          </button>
          <button onClick={resetFilters} disabled={loading} title="Reset all filters"
      ...
      <div ref={resultsRef}>
        ...
        {loading && (
          <div className="space-y-3 sm:space-y-4">
            {Array.from({ length: Math.min(resultCount, 3) }).map((_, i) => (
              <SkeletonCard key={i} />
            ))}
          </div>
        )}
        ...
        {/* Home cooked results */}
        {!loading && generatedMeal?.type === 'home_cooked' && generatedMeal.meals?.length > 0 && (
          <div className="space-y-3 sm:space-y-4">
            ...
            {generatedMeal.meals.map((meal, i) => (
              <HomeCookedCard
                ...
                regenLoading={regenLoading.has(i)}
              />
            ))}
          </div>
        )}
        ...
        {/* Fast food results */}
        {!loading && generatedMeal?.type === 'fast_food' && (
